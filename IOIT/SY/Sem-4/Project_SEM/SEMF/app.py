...
# app.config['SECRET_KEY'] = print(secrets.token_hex(16))
...
app.config['SECRET_KEY'] = '5e183e27a43a9b73a2c5b2ed08ed1f4f9fc87cb1ed4a30ddc89631e12c173b9a'
...
@app.route('/')
def home():
    return render_template('tc1.html' , view = 'home')
...
def signup():
    ...
    return render_template('tc1.html' , view = 'signup')
...
def login():
    ...
    return render_template('tc1.html' , view = 'login')

# Dashboard Route (Protected)
@app.route('/dashboard')
@login_required
def dashboard():
    user_recipes = Recipe.query.filter_by(health_condition=current_user.health_condition).all()
    print("Dashboard current_user:", current_user)
    print("Recipes for user:", user_recipes)
    return render_template('tc1.html', view='dashboard', recipes=user_recipes)
...
def recipes():
    ...
    if meal_type_filter:
        filters.append(Recipe.meal_type == meal_type_filter)
    if difficulty_filter:
        filters.append(Recipe.difficulty == difficulty_filter)

    recipes = Recipe.query.filter(*filters).all()
    return render_template('tc1.html', view='recipes', recipes=recipes)
